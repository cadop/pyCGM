...
def default_segment_dict():
    ...
    segment_dict = {'Head': ['RFHD', 'RBHD', 'LFHD', 'LBHD', 'REAR', 'LEAR'],
                    'Trunk': ['C7', 'STRN', 'CLAV', 'T10', 'RBAK', 'RSHO', 'LSHO'],
                    'Pelvis': ['SACR', 'RPSI', 'LPSI', 'LASI', 'RASI'], 'RThigh': ['RTHI', 'RTH2', 'RTH3', 'RTH4'],
                    'LThigh': ['LTHI', 'LTH2', 'LTH3', 'LTH4'], 'RShin': ['RTIB', 'RSH2', 'RSH3', 'RSH4'],
                    'LShin': ['LTIB', 'LSH2', 'LSH3', 'LSH4'], 'RFoot': ['RLFT1', 'RFT2', 'RMFT3', 'RLUP'],
                    'LFoot': ['LLFT1', 'LFT2', 'LMFT3', 'LLUP'], 'RHum': ['RMELB', 'RSHO', 'RUPA'],
                    'LHum': ['LMELB', 'LSHO', 'LUPA']}
...
def get_marker_location(pm, c):
    ...
    # Define the transformation matrix of the marker in cluster space, cluster to Marker
...
def transform_from_static(data, data_mapping, static, static_mapping, key, useables, s):
    ...
    mov_c = data[s][data_mapping[useables[0]]], data[s][data_mapping[useables[1]]], data[s][data_mapping[useables[2]]]

    return get_marker_location(pm, mov_c)
...
def transform_from_mov(data, data_mapping, key, clust, last_time, i):
    ...
    pm = get_static_transform(p, c)
    mov_c = data[i][data_mapping[clust[0]]], data[i][data_mapping[clust[1]]], data[i][data_mapping[clust[2]]]
    ...
    for key in data_mapping:
        ...
        for i, val in enumerate(traj):
            ...
            if last_time:
                ...
                data_copy[i][data_mapping[key]] = transform_from_mov(data_copy, data_mapping, key, useables, last_time,
                                                                     i)
                continue
            ...
            if not last_time:
                ...
                data_copy[i][data_mapping[key]] = transform_from_static(data_copy, data_mapping, static, static_mapping,
                                                                        key, useables, i)
